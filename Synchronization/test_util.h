...
/**
 * @brief Workload Type
 * INSERT_ONLY          : workload num 만큼 insert한 후, single-thread로 순회한다.
 * INSERT_LOOKUP        : workload num 만큼 insert(50%), lookup(50%)한 후, single-thread로 순회한다.
 * INSERT_LOOKUP_DELETE : workload num 만큼 insert(60%), lookup(20%), delete(20%)한다.
 *                        Single-thread인 경우에만 순회를 수행한다.
 */
...
/// @brief 부모 테스트 클래스
class HashTableTest : public ::testing::TestWithParam<std::tuple<WL_TYPE, int, int>> {
 protected:
    // 테스트 워크로드
    Req* workload_;
    // 테스트할 HashTable 포인터
 ...
 public:
    /// @brief TEST_P 실행 전, 실행되는 함수
    void SetUp() override;

    /// @brief TEST_P 실행 후, 실행되는 함수
     ...
     * HashTable과 std::map을 순회한 결과가 모두 동일한지 확인한다.
     */
    void check_answer();
};

/// @brief Test HashTable with Single Thread
class HashTableSingleThreadTest : public HashTableTest {};

/// @brief Test HashTable with Multi Thread
class HashTableMultiThreadTest : public HashTableTest {};
